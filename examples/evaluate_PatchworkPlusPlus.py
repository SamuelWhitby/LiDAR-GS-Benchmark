...
## Algorithm Name ##
algorithm_name = 'PatchworkPlusPlus'   #<----------------------------------------------------------------------------------------
...
params.verbose = False  # Print evaluation
        ...
        for attribute_folder in os.listdir(environment_path):
            ...
            for group_folder in sorted(os.listdir(attribute_path)):
                ...
                for f in range(len(frames)):
                    ...
                    PatchworkPlusPlus.estimateGround(pcd)
                    ground_idx = PatchworkPlusPlus.getGroundIndices()
                    nonground_idx = PatchworkPlusPlus.getNongroundIndices()
                    ground_estimate = np.zeros(pcd.shape[0], dtype=bool)
                    ground_estimate[ground_idx] = True
                    end_time = time.time()
